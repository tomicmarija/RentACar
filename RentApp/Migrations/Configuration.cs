namespace RentApp.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<RentApp.Persistance.RADBContext>
    {
        ...
        protected override void Seed(RentApp.Persistance.RADBContext context)
        {
            ...
            context.AppUsers.AddOrUpdate(u => u.Id, new AppUser() { Id = 8, FullName = "Milica Mitrovic", FirstName = "Milica", LastName = "Mitrovic", DateOfBirth = DateTime.Now, AllowCreating = true });
            ...
            if (!context.Users.Any(u => u.UserName == "milica"))
            {

                var _appUser = context.AppUsers.FirstOrDefault(a => a.FullName == "Milica Mitrovic");
                var user = new RAIdentityUser() { Id = "milica", UserName = "milica", Email = "milica__@yahoo.com", PasswordHash = RAIdentityUser.HashPassword("milica"), AppUserId = _appUser.Id };
                userManager.Create(user);
                userManager.AddToRole(user.Id, "Manager");
